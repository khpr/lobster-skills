...
name: telegram-media
description: |
  處理 Telegram 收到的圖片、語音、影片、檔案。
  
  觸發：[自動] 收到 Telegram 圖片/語音/影片/檔案。
  不觸發：要主動發送媒體（用 telegram-output）、處理 LINE 媒體（用 line-media）。
...
```bash
# Step 1: 取得 file_path
BOT_TOKEN=$(python3 -c "import json; d=json.load(open('$HOME/.openclaw/openclaw.json')); print(d['channels']['telegram']['botToken'])")
...
Telegram inbound 媒體自動存到：`~/.openclaw/media/inbound/`（若 OpenClaw 已配置自動下載）
...
VAULT_MEDIA="$HOME/Library/Mobile Documents/iCloud~md~obsidian/Documents/Obsidian Vault/90_System/Deliverables/media"
...
```bash
bash $HOME/.openclaw/workspace/scripts/file-extract.sh /tmp/{filename}
```
